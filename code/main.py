...
def render_index():
    ...
    # TODO: boot them if they're signed in with a non-columbia id (insecure since it's client-side but whatever)
    # then if the user is signed in, check if they already have an account
    # if they don't, render the creation form with their first/last name info already filled in
    # if they do, take them to the listing page
    ...
    # then render the index page depending on whether the user has been signed in
    # we need the index page template to have a field to change for this
    # if no user: give the register page
    # if user: render the listings page
    return greeting
